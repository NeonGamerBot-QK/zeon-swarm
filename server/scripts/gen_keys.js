...
function generateCrypticName() {
  const prefixes = [
    "Echo", "Null", "Silent", "Crimson", "Hidden", "Obsidian",
    "Lunar", "Phantom", "Eternal", "Neon", "Aether", "Void",
    "Iron", "Shadow", "Glass", "Wired", "Signal", "Solar"
  ];
  ...
  const suffixes = [
    "Cipher", "Veil", "Horizon", "Code", "Fragment", "Node", "Origin",
    "Pulse", "Pattern", "Construct", "Bloom", "Shard", "Frame", "Mirror"
  ];
...
if(!name || !email) {
  ...
  email = `${name.split(/ +/).join("-")}@${os.hostname()}`
  // console.log(`Provide an name and email via the cli, eg: "Name here" machine-name@device`)
  // return;
  console.warn(`[WARNING] no name or email using autogenerated "${name}" ${email}`)
}
(async () => {
